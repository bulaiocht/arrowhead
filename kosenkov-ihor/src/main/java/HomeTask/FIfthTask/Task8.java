...
public class Task8 {
    ...
    public static void main(String[] args) {
        ...
        findFrequencyOfOccurrenceLetters(new String[]{"a", "b"});
    }
    ...
    public static void findFrequencyOfOccurrenceLetters(String[] arr){
        try (BufferedReader br = new BufferedReader(new FileReader("kosenkov-ihor/src/main/resources/song.txt"));
             BufferedWriter bw = new BufferedWriter(new FileWriter("kosenkov-ihor/src/main/resources/song_copy.txt"))) {
            ...
            while (true){
                ...
                for (int i = 0; i < words.length; i++) {
                    for (int j = 0; j < arr.length; j++) {
                        count[j] += new StringTokenizer(" " +words[i].toLowerCase() + " ", arr[j]).countTokens()-1;
